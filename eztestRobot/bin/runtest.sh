      ...
      ##     eztest.exe < $QAENV/bin/eztest_setup_xml
       ...
       if [ $G_UPDATE = 'on' ] ;
          ...
          if [ $XML_JOB = 'on' ] ;
            then
				echo
        ##    ezupdate $CONFIG ${test_result_dir}/${testunit}_xml.sum $testunit
          else
			echo
        ##    ezupdate $CONFIG ${test_result_dir}/${testunit}.sum $testunit
          fi
       fi
}
...
do
	...
	#export DS_WORK=$runtest/work
	#export UDS_WORK=$unix_runtest/work
#	export DS_WORK_LOC_MAP=$runtest/work
#	export DS_LOCAL_JS_LOC=$runtest
#	export DS_JS_WORK_LOC=$runtest/work
	...
	java -jar ${ROBOTHOME}/bin/${robotframeworkjar} -v ROBOTHOME:${ROBOTHOME} --noncritical InTestingSetup -d ${test_result_dir}/${testsuite} -o ${cases}_output.xml -l ${cases}_log.html -r ${cases}_report.html $runtest/testcase.robot
	java -jar ${ROBOTHOME}/bin/${jythonjar} bin/summary.py ${test_result_dir}/${testsuite}/${cases}_output.xml
done
#generate_report.sh $cases
